...
import pandas as pd
app = Flask(__name__)
datawoz = pd.read_csv('flask-backend/raw-data/datasetwoznormaliseerd.csv')
datawoz = datawoz.drop(columns=['geometry', 'Unnamed: 0'])
datamisdaad = pd.read_csv('flask-backend/raw-data/misdrijven.csv')
...
datademografie = pd.read_csv('flask-backend/raw-data/Demografie_wijken_data.csv')
...
datawerk = pd.read_csv('flask-backend/raw-data/werkgelegenheid_df.csv')
